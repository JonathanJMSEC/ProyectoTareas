   ...
   public class TareaService : ITareaService
    {
        ...
        public async Task<ServiceResponse<Tarea>> CrearTareaAsync(CrearTareaDTO tarea)
        {
            if (!ValidacionesTarea.EsTareaValida(tarea).Success)
                return ServiceResponse<Tarea>.Error("Los datos de la tarea son inválidos.");
        ...
        public async Task<ServiceResponse<Tarea>> ObtenerTareaPorIdAsync(string id)
        {
            if (!ValidacionesTarea.EsIdValido(id).Success)
                return ServiceResponse<Tarea>.Error("El ID de la tarea es inválido.");

            var tarea = await _context.GetByIdAsync(id);

            if (!ValidacionesTarea.EsTareaExistente(id, tarea).Success)
                return ServiceResponse<Tarea>.Error("La tarea no existe.");
